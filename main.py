...
class MainWindow(QMainWindow):
    ...
    def refresh_docker_containers(self):
        ...
        worker = DockerListContainersWorker(); worker.moveToThread(self.data_worker_thread)
        worker.finished.connect(self.on_docker_containers_listed); worker.error_received.connect(self.log)
        self.data_worker_thread.started.connect(worker.run); self.data_worker_thread.start()
    ...
    def start_remote_download(self):
        ...
        self.log(f"Connecting to {user}@{host}..."); self.data_worker_thread = QThread()
        ...
        worker.moveToThread(self.data_worker_thread)
        worker.finished.connect(self.on_data_fetch_complete); worker.error_received.connect(self.log_and_reset)
        self.data_worker_thread.started.connect(worker.run); self.data_worker_thread.start()
    ...
    def start_docker_copy(self):
        ...
        self.log(f"Copying '{volume_path}' from container..."); self.data_worker_thread = QThread()
